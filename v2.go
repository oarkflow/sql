...
func NewConfigurableMapper(mapping map[string]string) *ConfigurableMapper {
	return &ConfigurableMapper{
		mapping:         mapping,
		expressionCache: make(map[string]*govaluate.EvaluableExpression),
	}
}

func (cm *ConfigurableMapper) Name() string {
	return "ConfigurableMapper"
}
...
func buildDestConnStr(dest config.DataConfig) string {
	if dest.Type == "postgresql" {
		return fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable",
			dest.Host, dest.Port, dest.Username, dest.Password, dest.Database)
	} else if dest.Type == "mysql" {
		return fmt.Sprintf("%s:%s@tcp(%s:%d)/%s",
			dest.Username, dest.Password, dest.Host, dest.Port, dest.Database)
	}
	return ""
}
...
func buildSourceConnStr(src config.DataConfig) string {
	if src.Type == "postgresql" {
		return fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable",
			src.Host, src.Port, src.Username, src.Password, src.Database)
	...
	if cfg.Source.Type == "csv" || cfg.Source.Type == "json" {
		...
		for _, tableCfg := range cfg.Tables {
			...
			if err := etlJob.Run(ctx); err != nil {
				logrus.Fatalf("ETL job failed for table %s: %v", tableCfg.NewName, err)
			}
			if err := etlJob.Close(); err != nil {
				logrus.Errorf("Error closing ETL job for table %s: %v", tableCfg.NewName, err)
			}
			logrus.Infof("Migration for table %s complete.", tableCfg.NewName)
			db.Close()
		}
		...
		if err != nil {
			logrus.Fatalf("Failed to open source DB: %v", err)
		}
		defer srcDB.Close()
		for _, tableCfg := range cfg.Tables {
			if !tableCfg.Migrate {
		...
		logrus.Fatalf("Unsupported source type: %s", cfg.Source.Type)
	}
